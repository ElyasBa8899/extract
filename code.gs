...
function setupSheet() {
  ...
  if (!ss.getSheetByName("Users")) {
    ...
    u.appendRow(["ID", "Name", "Username", "Password", "Role", "DailyHours", "ThursdayHours", "TotalMonthlySalary"]);
    u.appendRow([1, "مدیر سیستم", "admin", "123", "admin", 8, 4, 10000000]); // Example values
  }
      ...
      items.forEach(function(l) {
        if (l.act == 'Entry') {
          if (state == 'OUT') {
            lastTime = l.ts;
            state = 'IN';
            dailyDelayMins += calculateStrictDelay(l.timeStr);
          }
        } else if (state == 'IN') {
...
function calculateStrictDelay(timeStr) {
  ...
  // شیفت صبح: 08:30
  var mStart = timeToMins(SHIFT_CONFIG.morning.start);
  var mEnd = timeToMins(SHIFT_CONFIG.morning.end);

  // شیفت عصر: 17:00
  var eStart = timeToMins(SHIFT_CONFIG.evening.start);

  // اگر ورود در بازه صبح است (قبل از ۱۴)
  if (timeMins < mEnd) {
    if (timeMins > mStart) {
      delay = timeMins - mStart;
    }
  }
  // اگر ورود در بازه عصر است (بعد از ۱۴)
  else {
    if (timeMins > eStart) {
      delay = timeMins - eStart;
    }
  }
...
function updateUserInfo(id, name, username, password, dailyHours, thursdayHours, totalMonthlySalary) {
  var s = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Users");
  var d = s.getDataRange().getValues();
  for (var i = 1; i < d.length; i++) {
    if (String(d[i][0]) == String(id)) {
      s.getRange(i + 1, 2, 1, 7).setValues([[name, username, password, "user", dailyHours, thursdayHours, totalMonthlySalary]]);
      return { success: true };
    }
  }
...
function adminSaveUser(name, username, password, dailyHours, thursdayHours, totalMonthlySalary) {
  SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Users").appendRow([new Date().getTime(), name, username, password, 'user', dailyHours, thursdayHours, totalMonthlySalary]);
  return { success: true };
}
