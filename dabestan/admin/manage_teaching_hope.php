...
if (!is_admin()) { // permission: manage_meetings
...
// This page is very similar to manage_in_service_trainings.php
// In a larger application, these could be merged into a single meeting management page.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['meeting_title'])) {
    ...
    if (empty($title) || empty($meeting_date)) {
        ...
        try {
            // NOTE: We are using 'other' as meeting_type for simplicity.
            // A dedicated 'teaching_hope' type could be added to the ENUM/CHECK constraint in the DB.
            if ($meeting_id) {
                $stmt = $pdo->prepare("UPDATE meetings SET title=?, meeting_date=?, location=?, status=? WHERE id=? AND meeting_type='other'");
                $stmt->execute([$title, $meeting_date, $location, $status, $meeting_id]);
                $message = "جلسه با موفقیت ویرایش شد.";
            } else {
                $stmt = $pdo->prepare("INSERT INTO meetings (title, meeting_date, location, status, meeting_type, created_by) VALUES (?, ?, ?, ?, 'other', ?)");
                $stmt->execute([$title, $meeting_date, $location, $status, $_SESSION['id']]);
                $message = "جلسه جدید امید تدریس با موفقیت ثبت شد.";
            }
...
if (isset($_GET['delete'])) {
    ...
    try {
        $stmt = $pdo->prepare("DELETE FROM meetings WHERE id = ? AND meeting_type='other'");
...
// Fetch all "teaching hope" meetings
$meetings = $pdo->query("
    SELECT m.*, u.full_name as creator_name
    FROM meetings m
    JOIN users u ON m.created_by = u.id
    WHERE m.meeting_type = 'other' -- Using 'other' for teaching hope
...
if (isset($_GET['edit'])) {
    $edit_id = $_GET['edit'];
    $stmt = $pdo->prepare("SELECT * FROM meetings WHERE id = ? AND meeting_type='other'");
...
<div class="page-content">
    <div class="container-fluid">
        <h2>مدیریت جلسات امید تدریس</h2>
        <p>جلسات مربوط به امید تدریس را در این بخش مدیریت کنید.</p>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="manage_teaching_hope.php" method="post">
                    <?php if ($edit_meeting): ?>
                    ...
                    <div class="form-group">
                        ...
                        <?php if ($edit_meeting): ?>
                            <a href="manage_teaching_hope.php" class="btn btn-secondary">انصراف</a>
                        <?php endif; ?>
                    </div>
                </form>
            </div>
        ...
        <div class="card mt-4">
            <div class="card-header"><h5>لیست جلسات امید تدریس</h5></div>
            <div class="card-body">
                            ...
                            <?php foreach ($meetings as $meeting): ?>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a href="manage_teaching_hope.php?edit=<?php echo $meeting['id']; ?>" class="btn btn-sm btn-warning">ویرایش</a>
                                        <a href="manage_teaching_hope.php?delete=<?php echo $meeting['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('آیا مطمئن هستید؟');">حذف</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
